...
public static class APIsServices
{
    ...
    // Speed Search!
    public static async Task<List<PipedTrack>?> PipedYouTubeSearch(string query) =>
        await Http.Client.GetFromJsonAsync<List<PipedTrack>>($"https://api.piped.private.coffee/search?q={Uri.EscapeDataString(query)}&filter=musics_songs");
